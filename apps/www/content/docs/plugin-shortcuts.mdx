...
description: Learn how to configure and use keyboard shortcuts in Late plugins.
---

This guide covers the configuration and usage of keyboard shortcuts in Late plugins, allowing you to create custom hotkeys for various editor actions.

## Plugin Shortcuts

Late provides a set of default shortcuts for common actions.
...
- Shortcuts can be customized or extended as needed in your Late configuration.
...
```ts
const FormattingPlugin = createLatePlugin({
  key: 'formatting',
  shortcuts: {
    toggleBold: {
      handler: ({ editor }) => {
