...
description: How to configure and customize Late plugins.
---

Late plugins are highly configurable, allowing you to customize their behavior to suit your needs. This guide will walk you through the most common configuration options and how to use them.

- [Getting Started: Components](/docs/getting-started#components) - Instructions for adding plugins to your editor
- [LatePlugin API](/docs/api/core/plate-plugin) - The complete API reference for creating plugins
...
```ts
const LinkPlugin = createLatePlugin({
  key: 'link',
  node: {
    isElement: true,
    isInline: true,
...
const ImagePlugin = createLatePlugin({
  key: 'image',
  node: {
    isElement: true,
    isVoid: true,
...
const BoldPlugin = createLatePlugin({
  key: 'bold',
  node: {
    isLeaf: true,
  },
...
Rather than render an element or a mark, you may want to customize the behavior of your editor. Various plugin options are available to modify the behavior of Late.

### Event Handlers

The recommended way to respond to user-generated events from inside a plugin is with the `handlers` plugin option. A handler should be a function that takes a `LatePluginContext & { event }` object.
...
```ts showLineNumbers
const ExamplePlugin = createLatePlugin({
  key: 'example',
  handlers: {
...
```ts showLineNumbers
const AlignPlugin = createLatePlugin({
  key: 'align',
  inject: {
    nodeProps: {
...
One common application of this technique is to create custom <Link href="/docs/normalization">normalizers</Link>.

```ts showLineNumbers {20}
const CustomNormalizerPlugin = createLatePlugin({
...
Late supports nested plugins, allowing you to create plugin hierarchies. Use the `plugins` property to define child plugins:

```ts
const ParentPlugin = createLatePlugin({
  key: 'parent',
  plugins: [
    createLatePlugin({ key: 'child1' }),
    createLatePlugin({ key: 'child2' }),
  ],
});
```
...
If you need to pass an explicit type as generic, you can use `createTLatePlugin`.

### Using createTLatePlugin

The `createTLatePlugin` function allows you to create a typed plugin:
...
const CodeBlockPlugin = createTLatePlugin<CodeBlockConfig>({
  key: 'code_block',
...
See the [LatePlugin API](/docs/api/core/plate-plugin) for more plugin options.