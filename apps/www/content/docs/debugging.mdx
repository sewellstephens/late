---
title: Debugging
description: Debugging in Late.
---

## Using the DebugPlugin

The `DebugPlugin` is automatically included when you create a Late editor. You can access its methods through the editor's API:
...
Besides using the DebugPlugin, there are other effective ways to debug your Late editor:
...
```ts
const LoggingPlugin = createLatePlugin({
  key: 'logging',
  extendEditor: (editor) => {
    const { apply } = editor;
...
3. If the issue persists, <Link href="https://github.com/sewellstephens/late/issues/new?assignees=&labels=bug&projects=&template=bug.yml">open an issue on GitHub</Link> or share your example on <Link href="https://discord.gg/mAZRuBzGM3">Discord</Link>.

## Debug Error Types

Late uses several predefined error types to help identify specific issues during development. Here's a list of these error types and their descriptions:
...
Thrown when a plugin wasn't created using `createSlatePlugin` or `createLatePlugin` function. This error occurs when a plugin is added to the editor without being properly created using the designated function.
