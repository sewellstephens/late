---
title: Late
description: API reference for Late component.
---

`Late` is the root component loading the editor state into a store provider.   

`LateContent` is the component that renders the editor.
...
<APIItem name="onChange" type="({ value, editor }: { value: Value; editor: LateEditor }) => void" optional>
...
Controls whether the editor is considered active by default when used with a LateController.
...
`Late` requires an `editor` prop, which should be an instance of `LateEditor`. If `editor` is `null`, `Late` will not render anything.

The `Late` component does not handle the creation of the editor or the management of plugins. These responsibilities are handled by `createLateEditor`.

`Late` provides a store for the editor state and renders its children. It uses `LateStoreProvider` to make the editor state and related functions available to its children components.
