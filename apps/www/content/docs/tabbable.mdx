...
```bash
npm install @sewell_stephens/late-tabbable
```

## Usage

```tsx
import { TabbablePlugin } from '@sewell_stephens/late-tabbable/react';
...
<APIOptions>
<APIItem name="query" type="(editor: LateEditor<V>, event: KeyboardEvent) => boolean" optional>
...
<APIItem name="insertTabbableEntries" type="(editor: LateEditor<V>, event: KeyboardEvent) => TabbableEntry[]" optional>
