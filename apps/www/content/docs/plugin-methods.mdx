...
description: Explore the various methods available for extending Late plugins.
---
...
```ts
const TablePlugin = createLatePlugin({
  key: 'table',
...
```ts
const CounterPlugin = createLatePlugin({
  key: 'counter',
  options: {
    count: 0,
  },
...
While there is currently no core difference between API and Transforms in Late, they serve distinct purposes and are designed with future extensibility in mind:
...
## Convert a Slate Plugin to a Late Plugin

To convert a typed Slate plugin to a Late plugin, you can use `toLatePlugin`:

```ts
const CodeBlockPlugin = toLatePlugin(createSlatePlugin({ key: 'code_block' }), {
