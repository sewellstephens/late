...
description: Explore the various methods available for interacting with and customizing the Late editor.
---

This guide covers the various methods available on the Late editor instance.
...
```ts
import { useEditorRef, useEditorSelector, useEditorState } from '@sewellstephens/plate-common/react';
...
To access the editor outside the `Late` component or work with multiple editors, use the `LateController` component:

```ts
import { LateController } from '@sewellstephens/plate-common/react';
...
`LateController` manages active editors:
- The first mounted editor is active by default (override with `primary={false}` on `Late`).
- Focus changes the active editor.
- An editor remains active until another is focused or it unmounts.

Hooks like `useEditorRef` and `useEditorSelector` work with the active editor inside `LateController`. If no editor is active, they return a fallback editor, which:
- Provides default values for queries.
- Cannot be mutated.
- Throws errors on state-changing operations.

Fallback editor scenarios:
- No mounted `Late` components.
- All `Late` components are non-primary.
- During `LateContent` mounting.
